...
export class LinkTester {
   ...
   */
  private static isProductPage(finalUrl: string, testType: 'nykaa' | 'tira'): boolean {
    switch (testType) {
      case 'nykaa':
        // Check for /p/ pattern or ?productId= parameter
        return finalUrl.includes('/p/') || finalUrl.includes('?productId=') || finalUrl.includes('&productId=');
   ...
   */
  static async testSingleItem(itemId: string, url: string, itemType: 'link' | 'product'): Promise<LinkTestResult> {
    ...
    try {
      ...
      if (statusCode >= 200 && statusCode < 400) {
        if (testType === 'standard') {
          ...
          logger.info(`Custom test result - Final URL: ${finalUrl}, Is Product Page: ${isWorking}`);
        }
      }
      
      logger.info(`${itemType} test result - URL: ${url}, Status: ${statusCode}, Working: ${isWorking}, Time: ${responseTime}ms, Final URL: ${finalUrl}`);
      ...
      logger.error(`${itemType} test failed - URL: ${url}, Error: ${errorMessage}, Time: ${responseTime}ms`);
   ...
   */
  static async testSingleLink(linkId: string, url: string): Promise<LinkTestResult> {
  ...
  static async testAllLinks(): Promise<LinkTestResult[]> {
    try {
      ...
      logger.info(`Found ${links.length} links and ${products.length} products to test`);
      ...
      for (let i = 0; i < links.length; i += batchSize) {
        ...
        logger.info(`Testing links batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(links.length / batchSize)}`);
      ...
      for (let i = 0; i < products.length; i += batchSize) {
        ...
        logger.info(`Testing products batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(products.length / batchSize)}`);
        
        const batchPromises = batch.map(product => 
          this.testSingleItem((product._id as any).toString(), product.affiliateUrl, 'product')
        );
      ...
      logger.info(`Testing job completed. Tested ${links.length} links and ${products.length} products`);
      return results;
  ...
  static async testLinksForUser(userId: string): Promise<LinkTestResult[]> {
    try {
      ...
      logger.info(`Found ${links.length} links and ${products.length} products for user ${userId}`);
      ...
      for (const link of links) {
        const result = await this.testSingleItem((link._id as any).toString(), link.url, 'link');
        results.push(result);
      ...
      for (const product of products) {
        const result = await this.testSingleItem((product._id as any).toString(), product.affiliateUrl, 'product');
        results.push(result);
      ...
      logger.info(`Testing completed for user ${userId}. Tested ${links.length} links and ${products.length} products`);
      return results;
      
    } catch (error) {
      logger.error(`Error testing links and products for user ${userId}:`, error);
      throw error;
    }
  }
   ...
   */
  private static async updateItemsStatus(results: LinkTestResult[]): Promise<void> {
    try {
      ...
      if (linkResults.length > 0) {
        ...
        logger.info(`Updated ${linkBulkResult.modifiedCount} links with test results`);
        
        const linkWorkingCount = linkResults.filter(r => r.isWorking).length;
        const linkNotWorkingCount = linkResults.filter(r => !r.isWorking).length;
        logger.info(`Link test summary: ${linkWorkingCount} working (active), ${linkNotWorkingCount} not working (deactivated)`);
      }
      ...
      if (productResults.length > 0) {
        ...
        logger.info(`Updated ${productBulkResult.modifiedCount} products with test results`);
        
        const productWorkingCount = productResults.filter(r => r.isWorking).length;
        const productNotWorkingCount = productResults.filter(r => !r.isWorking).length;
        logger.info(`Product test summary: ${productWorkingCount} working, ${productNotWorkingCount} not working`);
      }
   ...
   */
  private static async updateLinksStatus(results: LinkTestResult[]): Promise<void> {
  ...
  static async getLinkTestStats(): Promise<{
    ...
    try {
      ...
      const productNotWorking = await Product.countDocuments({ isWorking: false });
      
      const recentProduct = await Product.findOne({ lastTested: { $exists: true } })
        .sort({ lastTested: -1 })
        .select('lastTested');
