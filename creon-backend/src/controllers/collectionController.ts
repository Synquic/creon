...
export const createCollection = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const maxOrder = await ProductCollection.findOne({ userId }).sort({ order: -1 });
...
export const updateCollection = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (products !== undefined) {
      ...
      const currentCollection = await ProductCollection.findOne({ _id: id, userId });
...
export const addProductToCollection = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
