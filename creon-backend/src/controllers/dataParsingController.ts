...
export const fetchResponse = async (
  ...
  try {
    ...
    if (!html) {
      res.status(400).json({
        success: false,
        message: "Missing HTML content in request body.",
      });
      return;
    }

    // Check cache first using URL if provided
    if (url && urlCache.has(url)) {
      const cached = urlCache.get(url);
      if (cached) {
        res
          .status(200)
          .json({ success: true, message: "From cache", data: cached });
        return;
      }
    }

    logger.info("Received HTML length:", html.length);
    ...
    // Cache the result using URL if provided, otherwise use HTML hash
    const cacheKey = url || html.substring(0, 100); // Use first 100 chars of HTML as fallback key
    urlCache.set(cacheKey, parsed); // cache the result
    res.status(200).json({
      success: true,
