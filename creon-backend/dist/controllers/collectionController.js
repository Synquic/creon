...
const createCollection = async (req, res) => {
    try {
        ...
        const maxOrder = await models_1.ProductCollection.findOne({ userId }).sort({ order: -1 });
...
const updateCollection = async (req, res) => {
    try {
        ...
        if (products !== undefined) {
            ...
            const currentCollection = await models_1.ProductCollection.findOne({ _id: id, userId });
