...
const CollectionsPageNew: React.FC = () => {
  ...
  const toggleCollectionMutation = useMutation({
    mutationFn: ({
      ...
      title: string;
      isActive: boolean;
    }) => collectionService.updateCollection(id, title, { isActive }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["collections"] });
  ...
  const handleDelete = async (collectionId: string) => {
    if (
      confirm(
        "Are you sure you want to delete this collection? Products in this collection will not be deleted, but will be unassigned."
      )
    ) {
                  ...
                  <div className="relative">
                    <div className="w-32 h-32 bg-gray-100 rounded-xl flex items-center justify-center overflow-hidden">
                      {currentImage ? (
                        ...
                        <BoxesIcon className="w-12 h-12 text-gray-400" />
                      )}
                    </div>
      ...
      {collectionsLoading ? (
        ...
        <div className="text-center py-16">
          <div className="w-24 h-24 bg-gradient-to-br from-primary-100 to-primary-200 rounded-full mx-auto mb-6 flex items-center justify-center">
            <BoxesIcon className="w-12 h-12 text-primary-600" />
          </div>
