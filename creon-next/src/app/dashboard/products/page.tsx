...
const ProductsPageNew: React.FC = () => {
  ...
  const [pendingParsedData, setPendingParsedData] = useState<Product | null>(
    null
  );
  ...
  const toggleProductMutation = useMutation({
    mutationFn: ({ id, isActive }: { id: string; isActive: boolean }) =>
      ProductsService.updateProduct(id, { isActive }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["products"] });
    },
    onError: (error: string) => {
  ...
  const handleEdit = (product: Product) => {
    ...
    setValue("image", product.productImage || "");
    setCurrentImage(product.productImage || null);
