...
const LinkAnalyticsPage: React.FC = () => {
  ...
  const { linkId } = { linkId: "test" }; //useParams<{ linkId: string }>();
  ...
  const { isLoading } = useQuery({
    ...
    queryFn: () => analyticsService.getLinkAnalytics(linkId!, selectedPeriod),
    enabled: !!linkId,
  });
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-3">
            <Button
              ...
              size="sm"
              onClick={() =>
                window.open(`/s/${analyticsData.link.shortCode}`, "_blank")
              }
            >
              Visit Link
            </Button>
        ...
        >
          <div className="flex items-center space-x-3">
            <div className="w-12 h-12 bg-primary-100 rounded-xl flex items-center justify-center">
              <MousePointerClickIcon className="w-6 h-6 text-primary-700" />
            </div>
            <div>
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {Math.max(
                  ...Object.values(analyticsData.analytics.dailyClicks)
                )}
              </p>
            </div>
          </div>
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {analyticsData.analytics.recentClicks.map((click, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {click.userAgent.includes("Chrome")
                      ? "Chrome"
                      : click.userAgent.includes("Safari")
                      ? "Safari"
                      : "Other"}
                  </td>
                </tr>
