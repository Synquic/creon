...
const AppearancePageNew: React.FC = () => {
  ...
  const { data: themeData, isLoading } = useQuery({
    queryKey: ['theme'],
    queryFn: () => simpleApi.getUserTheme(),
  });

  const updateThemeMutation = useMutation({
    mutationFn: simpleApi.updateTheme,
    onSuccess: () => {
  ...
  const resetThemeMutation = useMutation({
    mutationFn: simpleApi.resetTheme,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['theme'] });
  ...
  const tabs = [
    ...
    { id: 'buttons', name: 'Buttons', icon: DevicePhoneMobileIcon },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mb-8">
              ...
              <p className="text-gray-600">Customize how your profile looks to visitors</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              {activeTab === 'colors' && (
                ...
                >
                  ...
                  {/* Predefined Palettes */}
                  <div className="mb-8">
                    <h4 className="text-sm font-medium text-gray-700 mb-4">Quick Palettes</h4>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <ColorPicker
                        ...
                        onChange={(value) => handleThemeUpdate({ textColor: value })}
                      />
                      <ColorPicker
                        label="Accent Color"
                  ...
                  <div className="space-y-6 mt-8">
                    <h4 className="text-sm font-medium text-gray-700">Background Gradient</h4>
                    <div className="space-y-4">
                      ...
                      <div>
                        <label className="flex items-center space-x-3">
                          <input
                            ...
                            onChange={(e) => handleThemeUpdate({ backgroundGradient: e.target.checked })}
                            className="w-5 h-5 text-primary-600 border-gray-300 rounded focus:ring-primary-500"
                          />
                          <span className="text-sm font-medium text-gray-700">Enable Background Gradient</span>
                        </label>
                      </div>
                      ...
                      {theme.backgroundGradient && (
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-3">Gradient Direction</label>
                          <div className="flex space-x-3">
                            {[
                              ...
                              <button
                                key={direction.value}
                                onClick={() => handleThemeUpdate({ gradientDirection: direction.value })}
                                className={`px-4 py-2 rounded-lg border-2 transition-colors ${
                                  (theme.gradientDirection || 'vertical') === direction.value
                                    ? 'border-primary-500 bg-primary-50 text-primary-700'
                                    : 'border-gray-200 hover:border-gray-300'
                                }`}
                              >
                                <span className="text-lg mr-2">{direction.icon}</span>
                                {direction.label}
                              </button>
                            ))}
                          </div>
                      ...
                      {theme.backgroundGradient && (
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-3">Gradient Preview</label>
                          <div 
                            ...
                            style={{
                              ...
                              }, ${theme.backgroundColor || '#ffffff'}, ${theme.secondaryColor || '#15803d'})`
                            }}
                          />
                        </div>
                      )}
                    ...
                    <div>
                      ...
                      <div className="flex space-x-3 flex-wrap gap-2">
                        {['light', 'normal', 'medium', 'semibold', 'bold'].map((weight) => (
                          <button
                            key={weight}
                            onClick={() => handleThemeUpdate({ fontWeight: weight })}
                            className={`px-4 py-2 rounded-lg border-2 transition-colors ${
              ...
              {activeTab === 'buttons' && (
                <motion.div
                  ...
                  transition={{ duration: 0.3 }}
                >
                  <h3 className="text-lg font-semibold text-gray-900 mb-6">Button Style</h3>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="flex space-x-3">
                        {[
                          ...
                          <button
                            key={style.value}
                            onClick={() => handleThemeUpdate({ buttonStyle: style.value })}
                    ...
                    {/* Button Animation */}
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-3">Button Animation</label>
                      <div className="grid grid-cols-2 gap-3">
                        {[
                          ...
                          <button
                            key={animation.value}
                            onClick={() => handleThemeUpdate({ buttonAnimation: animation.value })}
                            className={`p-3 border-2 rounded-lg transition-all ${
                              (theme.buttonAnimation || 'hover-scale') === animation.value
                                ? 'border-primary-500 bg-primary-50 text-primary-700'
                                : 'border-gray-200 hover:border-gray-300'
                            } ${
                              animation.value === 'hover-lift' ? 'hover:-translate-y-1' :
                              animation.value === 'hover-scale' ? 'hover:scale-105' :
                              animation.value === 'hover-glow' ? 'hover:shadow-lg' : ''
                            }`}
                          >
                            {animation.label}
                          </button>
                    ...
                    <div>
                      <label className="flex items-center space-x-3">
                        <input
                          ...
                          onChange={(e) => handleThemeUpdate({ buttonShadow: e.target.checked })}
                          className="w-5 h-5 text-primary-600 border-gray-300 rounded focus:ring-primary-500"
                        />
                        <span className="text-sm font-medium text-gray-700">Enable Button Shadow</span>
                      </label>
                    </div>
                  </div>
                </motion.div>
              ...
              {activeTab === 'layout' && (
                ...
                >
                  ...
                  <div className="space-y-6">
                    {/* Profile Image Shape */}
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-3">Profile Image Shape</label>
                      <div className="flex space-x-3">
                        {[
                          ...
                          <button
                            key={shape.value}
                            onClick={() => handleThemeUpdate({ profileImageShape: shape.value })}
                            className={`px-4 py-2 border-2 rounded-lg transition-colors ${
                              (theme.profileImageShape || 'circle') === shape.value
                                ? 'border-primary-500 bg-primary-50 text-primary-700'
                                : 'border-gray-200 hover:border-gray-300'
                            }`}
                          >
                    ...
                    {/* Link Spacing */}
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-3">Link Spacing</label>
                      <div className="flex space-x-3">
                        {[
                          ...
                          <button
                            key={spacing.value}
                            onClick={() => handleThemeUpdate({ linkSpacing: spacing.value })}
                            className={`px-4 py-2 border-2 rounded-lg transition-colors ${
                    ...
                    <div>
                      <label className="flex items-center space-x-3">
                        <input
                          ...
                          onChange={(e) => handleThemeUpdate({ backdropBlur: e.target.checked })}
                          className="w-5 h-5 text-primary-600 border-gray-300 rounded focus:ring-primary-500"
                        />
                        <span className="text-sm font-medium text-gray-700">Enable Backdrop Blur</span>
                      </label>
                    </div>
                    ...
                    <div>
                      <label className="flex items-center space-x-3">
                        <input
                          ...
                          onChange={(e) => handleThemeUpdate({ hideBranding: e.target.checked })}
                          className="w-5 h-5 text-primary-600 border-gray-300 rounded focus:ring-primary-500"
                        />
                        <span className="text-sm font-medium text-gray-700">Hide Creon Branding</span>
                      </label>
